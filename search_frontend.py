...
def search():
    ...
    if len(tokenize(query)) in (1, 2):
        ...
        res = search_title_our(query, title_index, file_name)[:100]
    else:
...
def search_body():
    ...
    res = search_body_our(query, body_index, file_name)
    # END SOLUTION
    return jsonify(res)
...
def search_title():
    ...
    res = search_title_our(query, title_index, file_name)
    # END SOLUTION
    return jsonify(res)
...
def search_anchor():
    ...
    res = search_anchor_our(query, anchor_index, file_name)
    # END SOLUTION
    return jsonify(res)
...
def Search_BM25(queries):
    ...
    for term in tokens:
        if term in title_index.term_total and term in body_index.term_total and term == tokens[random_index]:
            list_titles = read_posting_list(title_index, term, file_name_title)
            docs_ids = []
            list_docid_tf_foreach_term = read_posting_list(body_index, term, file_name_body)

            for doc_id, freq in merge_lists(list_docid_tf_foreach_term, list_titles):
                term_docid_freq[(term, doc_id)] = freq * 20
                docs_ids.append(doc_id)

            relevant_docs += docs_ids
        else:
            if term in body_index.term_total:

                list_docid_tf_foreach_term = read_posting_list(body_index, term,
                                                               '/content/drive/MyDrive/Proj_IR_23/body_pos/')
    ...
    doc_id_bm25 = [(doc_id, BM25_score_docid_query(dict(Counter(tokens)), doc_id)) for doc_id in set(relevant_docs)] # 1.5,0.75
